...
<script setup>
    ...
    const submitForm = () => {
        ...
        if (!errors.value.name && !errors.value.email && !errors.value.phone && !errors.value.message && !errors.value.agrement) {
            ...
            axios.post('https://api.test.cyberia.studio/api/v1/feedbacks', payload)
            .then(res => {
                response.value = res.date;
                console.log('Успешный POST-запрос:', res.data);
            })
            .catch(error => {
            ...
            console.log('Форма отправлена', formDate.value);
        } else {
...
<style lang="scss">
    ...
    .checkbox {
        ...
        &__input + &__span::before {
            ...
            transition: background 0.3s, border-color 0.3s; /* добавляем плавный переход */
            user-select: none;
        }
